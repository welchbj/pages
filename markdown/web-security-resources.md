---
title: web security resources
description: a collection of articles and guides for HTTP / web security
published_at: 2019-08-13
last_modified_at: 2019-08-13
---

# Web security resources

## General resources

* [PortSwigger's web appliction hacker's handbook](https://portswigger.net/web-security/web-application-hackers-handbook)
* [ngalongc's bug bounty reference](https://github.com/ngalongc/bug-bounty-reference)
* [great blog with lots of bug bounty writeups](https://ngailong.wordpress.com/)

## Headers

* [Twilio's HTTP headers for the responsible developer](https://www.twilio.com/blog/a-http-headers-for-the-responsible-developer)
* [Everything you need to know about HTTP security headers](https://blog.appcanary.com/2017/http-security-headers.html)

## HTTP Strict Transport Security (HSTS)

* [Testing the HSTS preload process](https://scotthelme.co.uk/hsts-preload-test/)
* [Using security features to do bad things](https://scotthelme.co.uk/using-security-features-to-do-bad-things/)

## Cross-Origin Request Sharing (CORS)

* [Developers don't understand CORS](https://fosterelli.co/developers-dont-understand-cors)

## Cross-Site Scripting (XSS) / Cross-Site Script Inclusion (XSSI)

* [Stack Overflow - what is XSSI?](https://stackoverflow.com/questions/8028511/what-is-cross-site-script-inclusion-xssi)
* [Cross-site script inclusion: A fameless but widespread web vulnerability class](https://www.scip.ch/en/?labs.20160414)

## Open redirects

* [Stack Overflow - URL fragment and 302 redirects](https://stackoverflow.com/questions/2286402/url-fragment-and-302-redirects/2305927)
* [Login CSRF + open redirect = account take over](https://ngailong.wordpress.com/2017/08/07/uber-login-csrf-open-redirect-account-takeover/)
* [OWASP - unvalidated redirects and forwards cheat sheet](https://github.com/OWASP/CheatSheetSeries/blob/master/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.md)
* [PayloadsAllTheThings - Open URL redirection](https://github.com/swisskyrepo/PayloadsAllTheThings/tree/master/Open%20Redirect)
* [Authentication bypass on Airbnb via OAuth tokens theft](https://www.arneswinnen.net/2017/06/authentication-bypass-on-airbnb-via-oauth-tokens-theft/)

## Cache poisoning / request smuggling

* [HTTP desync attacks: request smuggling reborn](https://portswigger.net/blog/http-desync-attacks-request-smuggling-reborn)

## Insecure direct object reference (IDOR)

* [IDOR (Insecure direct object reference)](https://www.bugbountynotes.com/mobile/tutorial?id=2)

## Creative injections

* [CRLF injection on Twitter or why blacklists fail](https://blog.innerht.ml/twitter-crlf-injection/)

## URI exploitation

* [Abusing of protocols to upload local files, bypass the HTML5 sandbox, and open popups (Edge)](https://www.brokenbrowser.com/abusing-of-protocols/)

## Scanning

* [Exposing intranets with reliable browser-based port scanning](https://portswigger.net/blog/exposing-intranets-with-reliable-browser-based-port-scanning)
